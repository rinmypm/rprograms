...
animate_plot <- function(x) {
  for (i in seq(1, iter)) {
    ...
    pl <- ggplot(temp, aes(x = Generation, y = Survivalpoints, group = Variable, 
        ...
        scale_y_continuous(limits = c(0, 110)) + geom_hline(y = max(temp$Survivalpoints), lty = 2) + 
        annotate("text", x = 1, y = max(temp$Survivalpoints) + 2, hjust = 0, size = 3, 
        color = "black", label = paste("Best solution:", max(temp$Survivalpoints))) + 
...
saveHTML(animate_plot(), interval = 0.1, outdir = getwd())
